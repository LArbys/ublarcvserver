...
export PYTHONPATH=${ubdl_dir}/ublarcvserver/networks/larflow/models/:$PYTHONPATH

let line=${SLURM_ARRAY_TASK_ID}+1
devnum=`sed -n ${line}p ${gpu_list}`
#device=`printf cuda:%d ${devnum}`
device=cuda
log_file=`printf ${log_dir}/dense_larflow_worker_id%d.log ${SLURM_ARRAY_TASK_ID}`
echo "SETUP WOKER TO RUN ON DEVICE=${device}"
script=${larflow_dir}/./start_dense_ublarflow_worker.py
